...
replaceOnce(
  "  function run(){",
  `  function classifyGraphic(svg){\n    var parts=[];\n    var node=svg;\n    for(var depth=0;node&&depth<5;depth++,node=node.parentElement){\n      parts.push(node.getAttribute&&node.getAttribute('class')||'');\n      parts.push(node.getAttribute&&node.getAttribute('aria-label')||'');\n    }\n    var question=svg.closest('.question-block');\n    if(question)parts.push((question.textContent||'').slice(0,500));\n    var context=parts.join(' ').toLowerCase();\n    var graphicText=(svg.textContent||'').trim();\n    if(/bar-chart|\\bchart\\b|ratio-graph|graph-container|דיאגרמ/.test(context))return 'chart';\n    if(/geo-|geometry|triangle|trapezoid|angle|משולש|טרפז|זווית|גאומטר/.test(context))return 'geometry';\n    if(svg.querySelector('polygon')&&/°/.test(graphicText))return 'geometry';\n    if(/number-line|ציר מספרים/.test(context))return 'number-line';\n    if(/bags-|bag|שקיות|שקים/.test(context))return 'ratio-model';\n    if(/bracelet|circles-|circle-option|חרוז|עיגול|גולות/.test(context))return 'counters';\n    if(/grid-|tile|משבצ|ריצוף/.test(context))return 'grid';\n    return 'generic';\n  }\n\n  function ensureGraphicViewBox(svg){\n    if(svg.hasAttribute('viewBox'))return;\n    try{\n      var box=svg.getBBox();\n      if(box&&box.width>0&&box.height>0){\n        var span=Math.max(box.width,box.height);\n        var pad=Math.max(4,Math.min(12,span*0.055));\n        var x=Math.round((box.x-pad)*1000)/1000;\n        var y=Math.round((box.y-pad)*1000)/1000;\n        var w=Math.round((box.width+pad*2)*1000)/1000;\n        var h=Math.round((box.height+pad*2)*1000)/1000;\n        svg.setAttribute('viewBox',x+' '+y+' '+w+' '+h);\n        svg.setAttribute('data-viewbox-source','content-bbox');\n        return;\n      }\n    }catch(_error){}\n    function numericLength(value){\n      if(!value||/%/.test(value))return NaN;\n      var n=parseFloat(value);\n      return Number.isFinite(n)&&n>0?n:NaN;\n    }\n    var w=numericLength(svg.getAttribute('width'));\n    var h=numericLength(svg.getAttribute('height'));\n    if(!(w>0&&h>0)){var r=svg.getBoundingClientRect();w=r.width;h=r.height;}\n    if(w>0&&h>0){\n      svg.setAttribute('viewBox','0 0 '+String(Math.round(w*1000)/1000)+' '+String(Math.round(h*1000)/1000));\n      svg.setAttribute('data-viewbox-source','dimensions-fallback');\n    }\n  }\n\n  function moveText(svg,label,x,y){\n    var texts=[].slice.call(svg.querySelectorAll('text'));\n    var node=texts.find(function(item){return (item.textContent||'').trim()===label;});\n    if(!node)return;\n    node.setAttribute('x',String(x));\n    node.setAttribute('y',String(y));\n  }\n\n  function normalizeGeometryMeasurementLabels(svg){\n    var changed=0;\n    [].forEach.call(svg.querySelectorAll('text'),function(node){\n      var raw=(node.textContent||'').trim();\n      var value=raw;\n      while(/^[A-Z]{2}\\s*=\\s*/.test(value)){value=value.replace(/^[A-Z]{2}\\s*=\\s*/,'').trim();}\n      if(value!==raw&&value){\n        node.textContent=value;\n        node.setAttribute('data-measurement-label','value-only');\n        changed++;\n      }\n    });\n    if(changed)svg.setAttribute('data-measurement-labels','value-only');\n  }\n\n  function setGraphicWidth(svg,width){\n    svg.style.width=String(width)+'px';\n    svg.style.height='auto';\n    svg.style.maxWidth='100%';\n  }\n\n  function polishGraphicLabels(svg){\n    var label=svg.getAttribute('aria-label')||'';\n    var values=[].map.call(svg.querySelectorAll('text'),function(node){return (node.textContent||'').trim();});\n    if(label.indexOf('מלבן BDFE')!==-1){\n      moveText(svg,'F',95,104);\n      moveText(svg,'5',61,99);\n      moveText(svg,'E',91,207);\n      moveText(svg,'15',109,236);\n      moveText(svg,'18',22,140);\n      svg.setAttribute('data-label-layout','inscribed-rectangle-v3');\n    }\n    if(label.indexOf('AB=10a')!==-1&&label.indexOf('BC=4a')!==-1){\n      moveText(svg,'E',38,79);\n      setGraphicWidth(svg,165);\n      svg.setAttribute('data-label-layout','midpoint-triangle-v2');\n    }\n    if(label.indexOf('שאורכו 6p ורוחבו 2p')!==-1){\n      setGraphicWidth(svg,240);\n      svg.setAttribute('data-label-layout','rectangle-challenge-v2');\n    }\n    if(svg.getAttribute('viewBox')==='0 0 200 80'&&values.indexOf('12')!==-1&&values.indexOf('4')!==-1){\n      moveText(svg,'12',95,12);\n      svg.setAttribute('data-label-layout','rectangle-half-v2');\n    }\n  }\n\n  function normalizeGraphics(){\n    var svgs=[].slice.call(document.querySelectorAll('.wb-page svg'));\n    svgs.forEach(function(svg,index){\n      var family=classifyGraphic(svg);\n      ensureGraphicViewBox(svg);\n      if(family==='geometry')normalizeGeometryMeasurementLabels(svg);\n      polishGraphicLabels(svg);\n      svg.classList.add('math-graphic');\n      svg.setAttribute('data-graphic-index',String(index+1));\n      svg.setAttribute('data-graphic-family',family);\n      svg.setAttribute('shape-rendering','geometricPrecision');\n      svg.setAttribute('text-rendering','geometricPrecision');\n      svg.setAttribute('preserveAspectRatio','xMidYMid meet');\n      svg.setAttribute('focusable','false');\n      if(svg.getAttribute('aria-hidden')!=='true'&&!svg.getAttribute('aria-label')){\n        var q=svg.closest('.question-block');\n        var p=q&&q.querySelector('p');\n        var text=p&&p.textContent?p.textContent.replace(/\\s+/g,' ').trim():'';\n        svg.setAttribute('role','img');\n        svg.setAttribute('aria-label',text?('תרשים: '+text.slice(0,140)):'תרשים מתמטי');\n      }\n    });\n    document.documentElement.dataset.graphicsNormalized='true';\n    document.documentElement.dataset.graphicsCount=String(svgs.length);\n  }\n\n  function run(){`,
  'graphics normalizer',
);
